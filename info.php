...
switch ($action){
    ...
    case 'post':
        ...
        <div class="container text-center mt-5 mb-3">
            <img src="img/news/'.$aNews["pictures"].'">
        </div>
    ...
    case 'news':
        ...
        echo '<div class="container-fluid mt-5 text-light d-flex justify-content-around flex-wrap align-content-start align-items-start">';

        if($aNews->num_rows > 0){
            while($row = $aNews->fetch_assoc()){
                echo '
                <div class="card bg-dark m-1 mt-5" style="width: 500px; height: 370px;">
                    <img src="img/news/'.$row["pictures"].'" style="object-fit: cover" height="250px" class="card-img-top" alt="...">
                    <div class="card-body">
                        <a href="?section=info&action=post&id='.$row["id_wall"].'" class="btn text-light"><h5 class="card-title">'.$row["header"].'</h5></a>
                         <p class="card-text" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">'.$row["post_text"].'</p>
                    </div>
                </div>
                ';
            }
